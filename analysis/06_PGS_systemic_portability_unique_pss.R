...
## =========================
## Step 3 — Systemic ΔAUC strip plot (pairs as rows)
## =========================

suppressPackageStartupMessages({ library(readr); library(dplyr); library(ggplot2); library(scales) })

delta_fp <- file.path(out_dir, paste0("tbl_delta_pairs_DISPLAY_", stamp, ".csv"))
bar_fp   <- file.path(out_dir, paste0("tbl_barplot_source_DISPLAY_", stamp, ".csv"))
stopifnot(file.exists(delta_fp), file.exists(bar_fp))

delta_df <- readr::read_csv(delta_fp, show_col_types = FALSE)
bar_src  <- readr::read_csv(bar_fp,   show_col_types = FALSE)

# Size: discovery Neff if available in 'tb', else sum of eval N
size_title <- "Discovery Neff"
if (exists("tb") && "train_neff" %in% names(tb)) {
  size_tbl <- tb %>% distinct(pgs_id, train_neff)
  delta_df <- delta_df %>% left_join(size_tbl, by = "pgs_id") %>% rename(size_val = train_neff)
} else {
  size_title <- "Eval N (sum)"
  size_tbl <- bar_src %>% group_by(pgs_id) %>% summarise(size_val = sum(n_eval, na.rm = TRUE), .groups="drop")
  delta_df <- delta_df %>% left_join(size_tbl, by = "pgs_id")
}

# Order pairs by frequency (matches README counts order)
pair_counts <- delta_df %>% count(pair, sort = TRUE)
pair_levels <- pair_counts$pair
delta_df <- delta_df %>% mutate(pair = factor(pair, levels = pair_levels))

# Overlays per pair (median + IQR); compute numeric row positions for geom_rect/segment
overlay_df <- delta_df %>%
  group_by(pair) %>%
  summarise(
    x_med = median(delta_auc, na.rm = TRUE),
    x_q1  = quantile(delta_auc, 0.25, na.rm = TRUE),
    x_q3  = quantile(delta_auc, 0.75, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(y_num = as.numeric(factor(pair, levels = pair_levels)))

library(dplyr); library(ggplot2)

# delta_df, pair_levels, overlay_df already created above

# Quick density for each row (Violin is optional; remove if you prefer only IQR band)
p_pairs_strip <- ggplot(delta_df, aes(x = delta_auc, y = pair)) +
  # distribution backdrop
  geom_violin(scale = "width", width = 0.9, fill = "grey92", color = NA, trim = TRUE) +
  # IQR band per row
  geom_rect(
    data = overlay_df,
    aes(xmin = x_q1, xmax = x_q3, ymin = y_num - 0.35, ymax = y_num + 0.35),
    inherit.aes = FALSE, alpha = 0.12, fill = "grey70", color = NA
  ) +
  # vertical zero + row median marker
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey55") +
  geom_segment(
    data = overlay_df,
    aes(x = x_med, xend = x_med, y = y_num - 0.45, yend = y_num + 0.45),
    inherit.aes = FALSE, color = "grey35", linewidth = 0.6
  ) +
  # points with bucket encodings
  geom_point(
    aes(color = train_bucket, shape = train_bucket, size = pmax(size_val, 1)),
    alpha = 0.85, position = position_jitter(height = 0.08, width = 0.001)
  ) +
  scale_y_discrete(limits = pair_levels, expand = expansion(mult = c(0.02, 0.06))) +
  scale_color_manual(values = pal_okabe, drop = FALSE, name = "Training bucket") +
  scale_shape_manual(values = shape_map, drop = FALSE, name = "Training bucket") +
  scale_size_continuous(
    trans = "log10",
    breaks = c(1e4, 1e5, 1e6),
    labels = c("10K","100K","1M"),
    range = c(1.8, 6),
    name = attr(delta_df$size_val, "label") %||% "Discovery Neff / Eval N"
  ) +
  labs(
    title = "Systemic portability: ΔAUC across ancestry pairs",
    subtitle = "Rows ordered by availability; vertical line marks ΔAUC = 0; dark tick = row median",
    x = "ΔAUC (B − A; label is ‘B vs A’)", y = NULL
  ) +
  theme_portability

ggsave(file.path(out_dir, paste0("dotplot_pairs_strip_", stamp, "_clean.pdf")),
       p_pairs_strip, width = 11, height = 7, device = cairo_pdf)
# Sanity print (should match README “Target pairs coverage”)
message("Pair counts plotted:")
print(pair_counts, n = Inf)

## =========================
## Step 4 — Trait-focused ΔAUC strip plots (pairs as rows, facets by EFO)
## =========================
suppressPackageStartupMessages({ library(readr); library(dplyr); library(forcats) })

delta_pairs_fp <- file.path(out_dir, paste0("tbl_delta_pairs_DISPLAY_", stamp, ".csv"))
eval_full_fp   <- file.path(out_dir, paste0("eval_df_full_",           stamp, ".csv"))
bar_src_fp     <- file.path(out_dir, paste0("tbl_barplot_source_DISPLAY_", stamp, ".csv"))

stopifnot(file.exists(delta_pairs_fp), file.exists(eval_full_fp), file.exists(bar_src_fp))

if (!exists("delta_all")) {
  delta_all <- readr::read_csv(delta_pairs_fp, show_col_types = FALSE)
  eval_full <- readr::read_csv(eval_full_fp,   show_col_types = FALSE) %>%
    distinct(pgs_id, efo_id, efo_label)
  
  # attach traits
  delta_all <- delta_all %>%
    inner_join(eval_full, by = "pgs_id")
  
  # attach size (discovery Neff if present in tb; else sum eval N)
  bar_src <- readr::read_csv(bar_src_fp, show_col_types = FALSE)
  if (exists("tb") && "train_neff" %in% names(tb)) {
    delta_all <- delta_all %>%
      left_join(tb %>% distinct(pgs_id, train_neff), by = "pgs_id") %>%
      rename(size_val = train_neff)
    size_title <- "Discovery Neff"
  } else {
    delta_all <- delta_all %>%
      left_join(
        bar_src %>% group_by(pgs_id) %>% summarise(size_val = sum(n_eval, na.rm = TRUE), .groups="drop"),
        by = "pgs_id"
      )
    size_title <- "Eval N (sum)"
  }
  
  # global pair order by frequency
  pair_levels <- delta_all %>% count(pair, sort = TRUE) %>% pull(pair)
  delta_all <- delta_all %>% mutate(pair = factor(pair, levels = pair_levels))
}

# ---- Now run the "group traits by number of pairs" pagination (unchanged) ----
suppressPackageStartupMessages({ library(dplyr); library(ggplot2); library(scales); library(stringr); library(purrr) })

trait_meta_pairs <- delta_all %>%
  filter(!is.na(delta_auc)) %>%
  group_by(efo_label) %>%
  summarise(
    n_pairs = n_distinct(pair),
    k_pgs   = n_distinct(pgs_id),
    med_abs = median(abs(delta_auc), na.rm = TRUE),
    .groups = "drop"
  )

delta_all <- delta_all %>%
  left_join(trait_meta_pairs, by = "efo_label") %>%
  mutate(n_pairs = ifelse(is.na(n_pairs), 0L, n_pairs))

pairs_levels <- delta_all %>%
  distinct(efo_label, n_pairs) %>%
...
facet_ncol      <- 3
facet_nrow      <- 3
traits_per_page <- facet_ncol * facet_nrow

# plot builder uses your existing palettes/theme_portability + size_title set above
plot_all_traits <- function(df_page, overlay_page, ncol = 3, file_title = NULL, file_subtitle = NULL) {
  ggplot(df_page, aes(x = delta_auc, y = pair)) +
    geom_violin(scale = "width", width = 0.9, fill = "grey92", color = NA, trim = TRUE) +
    geom_rect(data = overlay_page,
              aes(xmin = x_q1, xmax = x_q3,
                  ymin = as.numeric(pair) - 0.35, ymax = as.numeric(pair) + 0.35),
              inherit.aes = FALSE, alpha = 0.12, fill = "grey70", color = NA) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "grey55") +
    geom_segment(data = overlay_page,
                 aes(x = x_med, xend = x_med,
                     y = as.numeric(pair) - 0.45, yend = as.numeric(pair) + 0.45),
                 inherit.aes = FALSE, color = "grey35", linewidth = 0.6) +
    geom_point(
      aes(color = train_bucket, shape = train_bucket, size = pmax(size_val, 1)),
      alpha = 0.85, position = position_jitter(height = 0.08, width = 0.001)
    ) +
    facet_wrap(~ efo_facet, ncol = ncol, scales = "fixed") +
    scale_y_discrete(limits = levels(df_page$pair), expand = expansion(mult = c(0.02, 0.06))) +
    scale_color_manual(values = pal_okabe, drop = FALSE, name = "Training bucket") +
    scale_shape_manual(values = shape_map, drop = FALSE, name = "Training bucket") +
    scale_size_continuous(
      trans = "log10", breaks = c(1e4, 1e5, 1e6), labels = c("10K","100K","1M"),
      range = c(1.8, 6), name = size_title
    ) +
    labs(
      title    = file_title %||% "Portability across traits: ΔAUC by ancestry pair",
      subtitle = file_subtitle %||% "Pairs are rows; facets are EFO traits (grouped by #pairs)",
      x = "ΔAUC (B − A; label is ‘B vs A’)", y = NULL
    ) +
    theme_portability
}

manifest <- tibble(page_file = character(), page_no = integer(), n_pairs = integer(), efo_label = character())
page_counter <- 0L

for (np in pairs_levels) {
  traits_np <- trait_meta_pairs %>%
    filter(n_pairs == np) %>%
    arrange(desc(med_abs), desc(k_pgs), efo_label) %>%
    pull(efo_label)
  if (!length(traits_np)) next
  
  group_df <- delta_all %>%
    filter(efo_label %in% traits_np) %>%
    mutate(
      efo_facet = factor(efo_label, levels = traits_np),
      pair      = factor(pair, levels = levels(delta_all$pair))
    )
  
  overlay_grp <- group_df %>%
    group_by(efo_facet, pair) %>%
    summarise(
      x_med = median(delta_auc, na.rm = TRUE),
      x_q1  = quantile(delta_auc, 0.25, na.rm = TRUE),
      x_q3  = quantile(delta_auc, 0.75, na.rm = TRUE),
  ...
  n_traits_grp <- length(traits_np)
  n_pages_grp  <- ceiling(n_traits_grp / traits_per_page)
  
  for (j in seq_len(n_pages_grp)) {
    page_counter <- page_counter + 1L
    idx_start <- (j - 1) * traits_per_page + 1
    idx_end   <- min(j * traits_per_page, n_traits_grp)
    page_traits <- traits_np[idx_start:idx_end]
    
    df_page <- group_df  %>% filter(efo_facet %in% page_traits)
    ov_page <- overlay_grp %>% filter(efo_facet %in% page_traits)
    
    title_txt    <- sprintf("ΔAUC by ancestry pair — Traits with %d pair(s)", np)
    subtitle_txt <- sprintf("Facets: %d trait(s) on this page (%d×%d grid). Files ordered by #pairs desc.",
                            length(page_traits), facet_nrow, facet_ncol)
    
    p <- plot_all_traits(df_page, ov_page, ncol = facet_ncol,
                         file_title = title_txt, file_subtitle = subtitle_txt)
    
    outfile_pdf <- file.path(
      out_dir,
      paste0("dotplot_ALLtraits_byPairs_np", sprintf("%02d", np), "_", stamp,
             sprintf("_page%02d.pdf", j))
    )
    ggsave(outfile_pdf, p, width = 13, height = 10, device = cairo_pdf)
    message("Saved: ", outfile_pdf)
    
    manifest <- bind_rows(
      manifest,
      tibble(page_file = basename(outfile_pdf),
             page_no   = page_counter,
             n_pairs   = np,
             efo_label = page_traits)
    )
  }
}

manifest_fp <- file.path(out_dir, paste0("dotplot_ALLtraits_byPairs_manifest_", stamp, ".csv"))
readr::write_csv(manifest, manifest_fp)
message("Index written: ", manifest_fp)
